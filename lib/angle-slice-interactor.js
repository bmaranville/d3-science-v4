(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    define(["exports", "//dev.jspm.io/d3@5"], factory);
  } else if (typeof exports !== "undefined") {
    factory(exports, require("//dev.jspm.io/d3@5"));
  } else {
    var mod = {
      exports: {}
    };
    factory(mod.exports, global.d35);
  ...
  function angleSliceInteractor(state, x, y) {
    ...
    var dispatch = _d2.default.dispatch("update");
    var x = x || _d2.default.scaleLinear();
    var y = y || _d2.default.scaleLinear();
    ...
    var drag_center = _d2.default.drag().on("drag", dragmove_center).on("start", function () {
      _d2.default.event.sourceEvent.stopPropagation();
    });

    var drag_lines = _d2.default.drag().on("drag", dragmove_lines).on("start", function () {
      _d2.default.event.sourceEvent.stopPropagation();
    });
    ...
    function dragmove_corner(d) {
      var new_x = x.invert(_d2.default.event.x),
          new_y = y.invert(_d2.default.event.y);
      var vertex = parseInt(_d2.default.select(this).attr("vertex"));
    ...
    function dragmove_center() {
      state.cx = x.invert(x(state.cx) + _d2.default.event.dx);
      state.cy = y.invert(y(state.cy) + _d2.default.event.dy);
      interactor.update();
    }

    function dragmove_lines() {
      var new_angle = Math.atan2(y(state.cy) - _d2.default.event.y, _d2.default.event.x - x(state.cx));
      if (_d2.default.select(this).classed("centerline")) {
        state.angle_offset = new_angle;
      } else if (_d2.default.select(this).classed("upperline")) {
        state.angle_range = new_angle - state.angle_offset;
      } else if (_d2.default.select(this).classed("lowerline")) {
        state.angle_range = -(new_angle - state.angle_offset);
      }
      interactor.update();
    }
