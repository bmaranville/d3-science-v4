(function (global, factory) {
  ...
  function crosshairInteractor(state, x, y) {
    ...
    var dispatch = _d3.default.dispatch("update");
    var x = x || _d3.default.scaleLinear();
    var y = y || _d3.default.scaleLinear();
    ...
    var drag_center = _d3.default.drag().on("drag", dragmove_center).on("start", function () {
      _d3.default.event.sourceEvent.stopPropagation();
    });

    var drag_lines = _d3.default.drag().on("drag", dragmove_lines).on("start", function () {
      _d3.default.event.sourceEvent.stopPropagation();
    });
    ...
    function dragmove_center() {
      state.cx = x.invert(x(state.cx) + _d3.default.event.dx);
      state.cy = y.invert(y(state.cy) + _d3.default.event.dy);
      interactor.update();
    }

    function dragmove_lines() {
      if (_d3.default.select(this).classed("vertical")) {
        state.cx = x.invert(x(state.cx) + _d3.default.event.dx);
      } else if (_d3.default.select(this).classed("horizontal")) {
        state.cy = y.invert(y(state.cy) + _d3.default.event.dy);
      }
      interactor.update();
    }
