(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    define(['exports', '//dev.jspm.io/d3@5', './jquery-extend.js', './generate-id.js'], factory);
  } else if (typeof exports !== "undefined") {
    factory(exports, require('//dev.jspm.io/d3@5'), require('./jquery-extend.js'), require('./generate-id.js'));
  } else {
    var mod = {
      exports: {}
    };
    factory(mod.exports, global.d35, global.jqueryExtend, global.generateId);
  ...
  function editor(data) {
    ...
    var wirecurve = 0.67;
    var svg, container;
    var dispatch = _d2.default.dispatch("update", "draw_wires");
    ...
    */
    ...
    var wireaction = _d2.default.behavior.drag().on("dragstart.wire", wirestart).on("drag.wire", wirepull).on("dragend.wire", wirestop);
    ...
    function wirestart() {
      ...
      if (!_d2.default.select(module_el).classed("wireable")) {
        return;
      }
      _d2.default.event.sourceEvent.stopPropagation();
      _d2.default.select(this).classed("highlight", true);
      var terminal_id = _d2.default.select(this).attr("terminal_id");
      var module_index = _d2.default.select(module_el).attr("index");
      ...
      svg.selectAll(dest_selector).on("mouseenter", function () {
        ...
        _d2.default.select(this).classed("highlight", false);
      });
    ...
    function wirestop() {
      ...
      if (this.classList.contains("input")) {
        var new_src = _d2.default.select(".output.highlight");
        if (!new_src.empty()) {
          var module_index = _d2.default.select(new_src.node().parentNode.parentNode).attr("index");
          active_data.source = [parseInt(module_index), new_src.attr("terminal_id")];
        }
      } else if (this.classList.contains("output")) {
        var new_tgt = _d2.default.select(".input.highlight");
        if (!new_tgt.empty()) {
          var module_index = _d2.default.select(new_tgt.node().parentNode.parentNode).attr("index");
    ...
    function module(module_data) {
      ...
      var drag = _d2.default.behavior.drag().on("drag", dragmove).origin(function (a) {
        return { x: module_data.x, y: module_data.y };
      });

      function dragmove() {
        if (!_d2.default.select(this).classed("draggable")) {
          return;
        }
        module_data.x = Math.round(_d2.default.event.x / grid_spacing) * grid_spacing;
        module_data.y = Math.round(_d2.default.event.y / grid_spacing) * grid_spacing;
        group.attr("transform", "translate(" + module_data.x.toFixed() + "," + module_data.y.toFixed() + ")");
        draw_wires();
      }

      // create and append module HTML element:
      group = _d2.default.select(document.createElementNS("http://www.w3.org/2000/svg", "g")).datum(module_data).classed("module draggable wireable", true).style("cursor", "move").attr("transform", "translate(" + module_data.x.toFixed() + "," + module_data.y.toFixed() + ")").attr("x-origin", module_data.x.toFixed()).attr("y-origin", module_data.y.toFixed());
    ...
    function wire(wire_data) {
      ...
      _d2.default.select(connector).classed("wire", true).style("cursor", "crosshair").style("fill", "none").style("stroke-width", "2.5px").style("stroke", "red");
      return connector;
    }
    return editor;
  }
